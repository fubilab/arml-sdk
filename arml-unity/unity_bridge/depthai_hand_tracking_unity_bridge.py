...
parser_renderer.add_argument('--no-preview', '--no_preview', dest='no_preview',
                    action="store_true",
...
while True:
    ...
    if len(hands)==1:
        ...
        configs = [['pd_score','pd_box','rotation','lm_score','landmarks','handedness','label','world_landmarks','xyz','gesture'],['result','arr1']]  # List of fields to serialize for each object
    elif len(hands)==2:
        names = ['hand_0','hand_1','res2']
        objects = [hands[0],hands[1],test_object]
        configs = [['pd_score','pd_box','rotation','lm_score','landmarks','handedness','label','world_landmarks','xyz','gesture'],['pd_score','pd_box','rotation','lm_score','landmarks','handedness','label','world_landmarks','xyz','gesture'],['result','arr1']]  # List of fields to serialize for each object
    else:
        names = ['res2']
        objects = [test_object]
    ...
    if args.no_preview:
        if renderer.output:
            renderer.output.write(renderer.frame)
        key = -1
    else:
